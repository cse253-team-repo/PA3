...
class Vgg11(nn.Module):
    def __init__(self, num_classes, retrain):
        ...
        self.deconv1 = nn.Sequential(
            nn.ConvTranspose2d(1000, 512, kernel_size=4, stride=4),
            nn.BatchNorm2d(512),
        )
    ...
    def forward(self, x):
        x = self.encoder(x).reshape(-1,1000,1,1)
...
class IncepV3(nn.Module):
    ...
    def forward(self, x):
        x = self.encoder.forward(x)[0].reshape(-1,1000,1,1)
