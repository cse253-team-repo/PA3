...
train_ids = list(trainId_to_id.values())
...
class IOU:
    def __init__(self,n_class=len(train_ids)):

        self.union = torch.zeros(n_class).cuda()
        self.intersection = torch.zeros(n_class).cuda()
        self.n_class = n_class
    def UpdateIou(self,pred, target, output=False):
        """
...
def pixel_acc(pred, target):
    ...
    """
    correct = torch.sum(pred * target,dim=[0,1,2,3])
    # Number of pixels
    N = torch.sum(target,dim=[0,1,2,3])
